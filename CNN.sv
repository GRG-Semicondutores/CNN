...
logic signed [DATA_WIDTH-1:0] result1 [0:N_FILTERS_LAYER_1-1][0:N_OUT-1]; // ERRO: ConvLayer1 deveria fornecer ACC_WIDTH_FMAP=22 bits por elemento (com os parametros padrao), mas este mapa armazena somente DATA_WIDTH=8 bits.
...
logic signed [DATA_WIDTH-1:0] result2 [0:N_FILTERS_LAYER_2-1][0:N_OUT_2-1]; // ERRO: ConvLayer2 deveria fornecer ACC_WIDTH_FMAP=24 bits por elemento (com os parametros padrao), mas este mapa armazena somente DATA_WIDTH=8 bits.
...
logic signed [DATA_WIDTH-1:0] result_FC1 [0:N_NEURONS_1-1]; // ERRO: z de FCLayer/Neuron e acumulado (2*DATA_WIDTH + $clog2(N_FLAT) bits), portanto esta ligacao trunca para DATA_WIDTH sem Quantizer. **será utilizado o quantizer em breve.
logic signed [DATA_WIDTH-1:0] result_FC2 [0:N_NEURONS_2-1]; // ERRO: z da segunda FCLayer tambem e acumulado (2*DATA_WIDTH + $clog2(N_NEURONS_1) bits) e e truncado para DATA_WIDTH. **será utilizado o quantizer em breve.
